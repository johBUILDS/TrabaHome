...
const WorkerSignup = () => {
  ...
  const [aiStatus, setAiStatus] = useState(''); // AI extraction status
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/worker/register`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          phone: normalizePhilippineMobile(formData.phone),
      ...
      navigate(`/verify-email/worker?email=${encodeURIComponent(formData.email)}`);
    } catch (err) {
  ...
  const handleFileChange = (e, field) => {
    ...
    if (file) {
      ...
      if (field === 'idFront' || field === 'idBack') {
        ...
        const nextState = { ...formData, [field]: file };
        if (nextState.idFront && nextState.idBack) {
          runServerAiExtraction(nextState);
        }
      }
    }
  };
  ...
  // Lightweight client-side "AI" stub to unblock flow.
  ...
  const runServerAiExtraction = async (state) => {
    setAiStatus('Reading ID ...');
    try {
      const frontB64 = await fileToBase64(state.idFront);
      const backB64 = await fileToBase64(state.idBack);
      const resp = await fetch(`${API_URL}/worker/id-ocr`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          idType: state.idType,
      ...
      if (!resp.ok) throw new Error(data?.message || 'ID parsing failed');
      const f = data.fields || {};
      setFormData(prev => ({
        ...prev,
        idNumber: f.idNumber || prev.idNumber,
        surname: f.surname || prev.surname,
        givenName: f.givenName || prev.givenName,
        middleName: f.middleName || prev.middleName,
        dateOfBirth: f.dateOfBirth || prev.dateOfBirth,
        nationality: f.nationality || prev.nationality,
        sex: f.sex || prev.sex,
        placeOfBirth: f.placeOfBirth || prev.placeOfBirth,
        dateOfIssue: f.dateOfIssue || prev.dateOfIssue,
        expiryDate: f.expiryDate || prev.expiryDate,
        issuingAuthority: f.issuingAuthority || prev.issuingAuthority
      }));
      setAiStatus('Auto-filled from ID (editable)');
    } catch (err) {
      console.error(err);
      setAiStatus(err.message || 'Auto-fill failed; please type details manually.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F2] flex items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-3xl shadow-sm w-full max-w-[500px] p-8 relative">
        ...
        {serverError && (
          <div className="mb-4 text-[10px] font-bold text-red-500 text-center">{serverError}</div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="space-y-2 mt-4">
              ...
              {aiStatus && <p className="text-[10px] text-center text-blue-700 font-semibold">{aiStatus}</p>}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  className={`w-full p-3 border rounded-lg text-sm outline-none appearance-none bg-white ${errors.specialization ? 'border-red-500' : 'border-gray-300'}`}
                >
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-[#004A8C] text-white py-3 rounded-lg font-bold mt-4 disabled:opacity-60"
            >
