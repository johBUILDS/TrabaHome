...
export default function WorkerList() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#F9F6F2] text-[#0B3B68] font-sans flex flex-col">
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-6 py-8 h-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
          ...
          <div className="flex bg-white rounded-lg p-1 shadow-sm border border-gray-200">
            ...
            <button onClick={() => navigate(`/search${currentParams}`)} className="flex items-center gap-2 px-4 py-1.5 rounded-md bg-[#0B3B68] text-white shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredWorkers.length > 0 ? (
            filteredWorkers.map((worker) => (
              <div key={worker.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col sm:flex-row gap-5">
                <div className="w-full sm:w-32 h-32 flex-shrink-0 rounded-xl overflow-hidden bg-gray-200 cursor-pointer" onClick={() => navigate(`/profile/${worker.id}`)}>
                  <img src={worker.image} alt={worker.name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                </div>

                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-lg font-bold text-[#0B3B68] flex items-center gap-1 cursor-pointer hover:text-[#00AF91]" onClick={() => navigate(`/profile/${worker.id}`)}>
                        {worker.name} <ShieldCheckIcon className="w-4 h-4 text-blue-500" />
                      </h3>
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="bg-blue-50 text-[#0B3B68] text-xs font-bold px-2 py-1 rounded-md flex items-center gap-1 border border-blue-100">{getRoleIcon(worker.role)} {worker.role}</span>
                        <div className="flex items-center text-yellow-500 text-xs font-bold"><StarIcon className="w-3 h-3 mr-1" /> {worker.rating}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 mt-4">
                    <button onClick={() => navigate(`/profile/${worker.id}`)} className="flex-1 px-3 py-1.5 border border-[#0B3B68] text-[#0B3B68] text-xs font-bold rounded-full hover:bg-blue-50 transition-colors">View Profile</button>
                    <button onClick={() => openRequestModal(worker.name)} className="flex-1 px-3 py-1.5 bg-[#00AF91] text-white text-xs font-bold rounded-full hover:bg-[#009b80] transition-colors shadow-sm">Request Service</button>
                  </div>
                </div>
              </div>
            ))
          ...
          <div className="relative bg-white w-full max-w-[500px] rounded-xl shadow-xl p-8 border border-blue-400">
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              <div className="flex flex-col gap-2">
                ...
                <input type="date" className="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-500 outline-none" value={preferredDate} onChange={(e) => setPreferredDate(e.target.value)} />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-[#111]">Preferred time <span className="text-gray-400 font-normal">(optional)</span></label>
                <input type="time" className="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-500 outline-none" value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)} />
              </div>
            </div>
            <button className="w-full bg-[#00AF91] text-white py-3.5 rounded-full font-bold text-lg hover:bg-[#009b80] mb-4" onClick={handleSendRequest}>
              Send Request
            </button>
            <div className="flex gap-2 items-start text-[11px] text-gray-500 italic">🔒 By sending this request, you agree to share contact details after acceptance.</div>
          </div>
        </div>
      )}
      ...
      {isSuccessOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-[340px] rounded-[10px] shadow-2xl p-10 flex flex-col items-center text-center">
            ...
            <div className="w-10 h-10 bg-[#4CAF50] rounded-full flex items-center justify-center mb-4">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h2 className="text-[22px] font-bold text-[#111] mb-2">Request Sent!</h2>
            <p className="text-gray-600 text-[12px] mb-8 leading-relaxed max-w-[200px]">
              We've notified the worker via SMS. You'll be updated once they accept.
            </p>
            <button 
              className="w-full bg-[#003C71] text-white py-3 rounded-[30px] font-bold text-sm hover:opacity-90 transition-all active:scale-95" 
              onClick={() => {
